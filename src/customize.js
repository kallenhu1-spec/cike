...
run(async()=>{motions=Object.fromEntries((await api.call('motion-library')).filter(r=>r.action?.motionId).map(r=>[r.action.motionId,{id:r.action.motionId,label:r.action.title,illustration:r.action.illustration}]));await refresh();select(rows[0]);$('data-path').textContent=await api.call('data-location');});
